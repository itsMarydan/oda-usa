...
import { BookOpen, ArrowRight, Heart, GraduationCap } from 'lucide-react';
import { getLibraryPage } from '@/sanity/fetch';

export default async function LibraryFeature() {
  const libraryData = await getLibraryPage();
  const heroImage = (libraryData.hero as { image?: string }).image;
  const metrics = libraryData.impact?.metrics?.slice(0, 3) ?? [];
  return (
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            {metrics.length > 0 && (
              <div className="grid grid-cols-3 gap-4 mb-8">
                {metrics.map((m, i) => {
                  const Icon = [GraduationCap, BookOpen, Heart][i] ?? BookOpen;
                  return (
                    <div key={m.label} className="text-center p-4 bg-white/10 rounded-lg backdrop-blur-sm">
          ...
          <div className="relative">
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border-2 border-white/20">
              <div className="relative aspect-[4/3] bg-white/5 rounded-xl flex items-center justify-center mb-6 overflow-hidden">
                {heroImage ? (
                  <Image
                    src={heroImage}
                    alt={libraryData.hero?.title ?? 'Library'}
                    fill
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-sm text-white/40 mt-1">Upload an image in Studio</div>
                  </div>
                )}
              </div>
