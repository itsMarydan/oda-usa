        ...
        <div className="container-custom">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {eventsData.past.map((event) => {
              const eventImage = (event as { image?: string }).image;
              return (
              <div key={event.id} className="card overflow-hidden">
                <div className="relative bg-gray-200 aspect-video flex items-center justify-center">
                  {eventImage ? (
                    <Image src={eventImage} alt={event.title} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                  ) : (
              ...
              {eventsData.gallery.culturalEvents.map((photo) => (
                <div key={photo.id} className="group relative aspect-square bg-gray-200 rounded-lg overflow-hidden cursor-pointer hover:scale-105 transition-transform">
                  {(photo as { src?: string }).src ? (
                    <Image src={(photo as { src: string }).src} alt={photo.caption ?? ''} fill sizes="(max-width: 768px) 50vw, 25vw" className="object-cover" />
                  ) : (
