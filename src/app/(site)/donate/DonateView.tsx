...
import type projectsDataType from '@/data/projects.json';
...
type Props = { projectsData: typeof projectsDataType };

export default function DonateView({ projectsData }: Props) {
        ...
        <div className="container-custom">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            ...
            {projectsData.featured.filter(p => p.needsSupport).map((project) => {
              const allocation = (project as { donationAllocation?: string }).donationAllocation ?? project.id;
              const Icon = projectIcons[allocation] || Activity;
              return (
                <button
                  key={project.id}
                  onClick={() => setSelectedProject(allocation)}
                  className={`card p-6 text-left transition-all ${
                    selectedProject === allocation
                      ? 'ring-4 ring-oda-blue shadow-lg'
                      : 'hover:shadow-lg'
                  }`}
                >
