...
import { getLeadership } from '@/sanity/fetch';
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
            {leadershipData.executiveCommittee.map((member) => {
              ...
              <div key={member.id} className="card p-6 text-center">
                <div className="relative w-32 h-32 mx-auto mb-4 rounded-full overflow-hidden bg-gradient-to-br from-oda-blue to-oda-sky flex items-center justify-center text-white text-3xl font-bold">
                  {memberImage ? (
                    <Image src={memberImage} alt={member.name} fill sizes="128px" className="object-cover" />
                  ) : (
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {leadershipData.boardOfTrustees.map((trustee) => {
                const trusteeImage = (trustee as { image?: string }).image;
                return (
                <div key={trustee.id} className="card p-6 text-center">
                  <div className="relative w-24 h-24 mx-auto mb-4 rounded-full overflow-hidden bg-gradient-to-br from-oda-gold to-oda-cream flex items-center justify-center text-oda-brown text-2xl font-bold">
                    {trusteeImage ? (
                      <Image src={trusteeImage} alt={trustee.name} fill sizes="96px" className="object-cover" />
                    ) : (
