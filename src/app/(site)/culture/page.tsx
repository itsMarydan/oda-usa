...
import { getCulturePage, getSiteSettings } from '@/sanity/fetch';

export default async function CulturePage() {
  const [cultureData, organizationData] = await Promise.all([
    getCulturePage(),
    getSiteSettings(),
  ]);
  return (
        ...
        <div className="container-custom">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            {cultureData.heritageSpotlight.map((spotlight) => {
              const spotlightImage = (spotlight as { image?: string }).image;
              return (
              <div key={spotlight.id} className="card overflow-hidden">
                <div className="relative bg-gray-200 aspect-video flex items-center justify-center">
                  {spotlightImage ? (
                    <Image src={spotlightImage} alt={spotlight.title} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                  ) : (
                    <div className="text-center p-8">
                      <div className="text-4xl mb-2">📸</div>
                      <div className="text-sm text-gray-500">Upload an image in Studio</div>
                    </div>
                  )}
                </div>
                <div className="p-6">
